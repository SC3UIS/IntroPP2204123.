...
int main(int argc, char *argv[])
{
    ...
    // Generar el tamaño de la lista y la clave de búsqueda aleatoriamente en el proceso 0
    if (rank == 0) {
        list_size = rand() % 20 + 5; // Generar un tamaño de lista aleatorio entre 5 y 24
    ...
    // Transmitir el tamaño de la lista y la clave de búsqueda a todos los procesos
    MPI_Bcast(&list_size, 1, MPI_INT, 0, MPI_COMM_WORLD);
    MPI_Bcast(&key, 1, MPI_INT, 0, MPI_COMM_WORLD);

    // Calcular el tamaño local de la lista para cada proceso
    int local_size = list_size / size + (rank < list_size % size ? 1 : 0);
    int *local_list = (int *)malloc(local_size * sizeof(int));

    // Generar elementos de la lista aleatoriamente en cada proceso
    for (int i = 0; i < local_size; i++) {
        local_list[i] = rand() % 100; // Generar números aleatorios entre 0 y 99
    }

    // Cada proceso ordena su parte de la lista
    bubble_sort(local_list, local_size);

    // Combinar las listas locales para obtener una lista global en el proceso 0
    int *list = NULL;
    if (rank == 0) {
